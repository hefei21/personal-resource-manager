    ...
    <section v-if="pane !== 'list' || !isMobile" class="note-content" :class="{ 'note-content-edit': pane === 'edit' }" aria-label="笔记内容">
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <article v-if="pane === 'read'" class="note-article"><div class="note-article-meta">{{ selected?.category_name || '未分类' }} · {{ dateLabel(selected?.updated_at) }}</div><h1>{{ selected?.title }}</h1><div class="note-read-tags"><span v-for="name in selected?.tags" :key="name">{{ name }}</span></div><MdPreview :model-value="selected?.content || ''" :sanitize="sanitizeRichHtml" /></article>
          <div v-else class="note-editor">
            ...
            <MdPreview v-if="preview" :model-value="form.content" :sanitize="sanitizeRichHtml" class="note-edit-preview" />
            <MdEditor v-else v-model="form.content" :sanitize="sanitizeRichHtml" :toolbars="toolbars" :footers="[]" :preview="false" :read-only="saving" :auto-detect-code="false" :no-upload-img="true" @on-save="saveNote" class="note-md-editor" />
