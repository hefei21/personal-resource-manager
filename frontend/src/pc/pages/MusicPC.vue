  ...
  >
    ...
    <div class="drag-overlay" v-if="isDragging">
      <div class="drag-content">
        <NativeIcon name="upload" size="64" />
        <p>释放鼠标上传音乐文件</p>
      </div>
    </div>
    ...
    <div class="main-content">
      ...
      <div class="sidebar" v-if="currentPlaylist || playlists.length > 0">
        <div class="sidebar-header">
          ...
          <div class="sidebar-actions">
            ...
            <NativeButton size="small" variant="text" iconSize="1.3em" @click="showCreatePlaylistDialog = true" title="创建歌单" :disabled="isGuest">
      ...
      <div class="music-list-container">
        ...
        <!-- 歌单分页 -->
        <div class="pagination-wrapper" v-if="currentPlaylist && playlistTotal > 0">
          <NativePagination
            v-model:current="playlistPagination.current"
            v-model:pageSize="playlistPagination.pageSize"
            :total="playlistTotal"
            :pageSizeOptions="[30, 50, 100]"
    ...
    >
      <div class="upload-area">
        ...
        >
          <NativeIcon name="upload" size="48" />
          <p>点击或拖拽文件到此区域上传</p>
        ...
        <div class="upload-queue" v-if="uploadQueue.length > 0">
          <div class="queue-header">
            ...
            <NativeButton size="small" variant="text" @click="clearUploadQueue" :disabled="isUploading">清空</NativeButton>
          </div>
          <div class="queue-list">
            <div v-for="file in uploadQueue" :key="file.id" class="queue-item">
              <div class="file-info">
                ...
                <NativeTag v-if="file.isRecovered" theme="primary" size="small" variant="outline">后台上传中</NativeTag>
                <NativeTag v-if="file.canResume" theme="success" size="small" variant="outline">可续传</NativeTag>
                <NativeTag v-if="file.status === 'pending'" theme="default" size="small">待上传</NativeTag>
                <NativeTag v-if="file.status === 'waiting_file'" theme="warning" size="small">需选择文件</NativeTag>
                <NativeTag v-if="file.status === 'duplicate'" theme="warning" size="small">重复</NativeTag>
                <NativeTag v-if="file.status === 'skipped'" theme="default" size="small">已跳过</NativeTag>
                <NativeTag v-if="file.status === 'cancelled'" theme="default" size="small">已取消</NativeTag>
                <NativeTag v-if="file.status === 'error'" theme="danger" size="small">失败</NativeTag>
              </div>
              <!-- 重复信息 -->
              ...
              <div class="file-progress" v-if="file.status === 'uploading' || file.status === 'success'">
                <NativeProgress
                  ...
                  :status="file.status === 'error' ? 'error' : (file.status === 'success' ? 'success' : 'primary')"
                  size="small"
                />
              </div>
    ...
    <NativeDialog
      ...
      @confirm="saveSong"
    >
      <NativeForm :modelValue="editForm">
        <NativeFormItem label="标题">
          <NativeInput v-model="editForm.title" />
        </NativeFormItem>
        <NativeFormItem label="艺术家">
          <NativeInput v-model="editForm.artist" />
        </NativeFormItem>
        <NativeFormItem label="专辑">
          <NativeInput v-model="editForm.album" />
        </NativeFormItem>
      </NativeForm>
    </NativeDialog>
    ...
    <!-- 歌单编辑对话框 -->
    <NativeDialog
      v-model="showPlaylistManageDialog"
      header="编辑歌单"
      width="500px"
      :footer="false"
    ...
    >
      <div class="playlist-edit-container">
        ...
        <template v-if="editingPlaylistId">
          ...
          <!-- 名称 -->
          <NativeFormItem label="名称" required>
            <NativeInput v-model="playlistEditForm.name" placeholder="歌单名称" />
          </NativeFormItem>
        ...
        <!-- 创建新歌单入口 -->
        <NativeDivider />
        <NativeButton theme="primary" variant="outline" block @click="showCreatePlaylistDialog = true" :disabled="isGuest">
          <template #icon><NativeIcon name="plus" /></template>
          创建新歌单
        </NativeButton>
      </div>
    ...
    >
      <NativeForm :modelValue="playlistForm">
        <NativeFormItem label="名称" required>
          <NativeInput v-model="playlistForm.name" placeholder="歌单名称" />
        </NativeFormItem>
        <NativeFormItem label="描述">
          <NativeTextarea v-model="playlistForm.description" placeholder="歌单描述" />
        </NativeFormItem>
      </NativeForm>
    </NativeDialog>
    ...
    >
      <NativeRadioGroup v-model="selectedPlaylistId">
    ...
    <!-- 批量下载歌词确认对话框 -->
    <NativeDialog
      v-model="showBatchLyricsConfirmDialog"
      header="批量下载歌词"
      width="400px"
      @confirm="confirmBatchDownloadLyrics"
    >
      <p>将为 {{ selectedSongs.length }} 首音乐搜索并下载歌词，是否继续？</p>
    </NativeDialog>

    <!-- 歌词下载进度对话框 -->
    <NativeDialog
      v-model="showLyricsProgressDialog"
      title="批量获取歌词"
      width="600px"
      :footer="false"
      ...
      <div class="lyrics-progress-container">
        <div class="progress-header">
          <NativeProgress
            ...
            :status="lyricsProgress.status === 'completed' ? 'success' : 'primary'"
            size="large"
          />
        ...
        <div class="progress-status" v-if="lyricsProgress.status === 'running'">
          <NativeIcon name="loading" size="20" spin />
        ...
        <div class="progress-results" v-if="lyricsProgress.results.filter(r => !r.skipped).length > 0">
          <NativeDivider>下载结果</NativeDivider>
          <div class="results-list">
            ...
            >
              <div class="result-info">
                <NativeIcon :name="result.success ? 'check-circle' : 'close-circle'" size="16" />
              ...
              <div class="result-meta">
                <NativeTag v-if="result.success" theme="success" variant="light" size="small">{{ result.source }}</NativeTag>
        ...
        <div class="progress-actions" v-if="lyricsProgress.status === 'completed'">
          <NativeButton theme="primary" @click="showLyricsProgressDialog = false">关闭</NativeButton>
        </div>
      </div>
    </NativeDialog>
  </div>
...
// 歌单分页（单独的状态，与全部音乐分页独立）
const playlistPagination = ref({ current: 1, pageSize: 30 })
...
const showBatchLyricsConfirmDialog = ref(false)
...
// 表格列定义
const columns = [
  { key: 'title', dataIndex: 'title', title: '标题', width: 350, align: 'left' },
...
// 歌单分页切换
function handlePlaylistPageChange(pageInfo) {
  playlistPagination.value.current = pageInfo.current
  loadPlaylistSongs()
  // 滚动到顶部
  const scrollContainer = document.querySelector('.scrollable-content')
  if (scrollContainer) {
...
async function playPlaylist(playlist) {
  try {
    // 获取歌单全部歌曲（不分页）
    const response = await api.music.getPlaylistSongs(playlist.id, { page: 1, pageSize: 10000 })
...
async function startUpload() {
  ...
  for (const file of uploadQueue.value) {
    ...
    try {
      ...
      // 开始上传会话
      console.log(`[上传] 开始: ${file.name}, 大小: ${(file.size / 1024 / 1024).toFixed(2)}MB, 分片: ${totalChunks}个`)
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        console.log(`[上传分片] ${file.name} - 分片 ${i + 1}/${totalChunks}, 大小: ${(chunk.size / 1024 / 1024).toFixed(2)}MB`)
        ...
        try {
          ...
          console.error(`[上传失败] ${file.name} 分片 ${i + 1}/${totalChunks}:`, error)
          throw error
        }
      }
      
      // 合并分片
      console.log(`[上传] 合并分片: ${file.name}`)
  ...
  if (successCount > 0) {
    toast.success(`上传完成，${successCount} 首音乐已添加`)
  }
  if (skippedCount > 0) {
    toast.info(`${skippedCount} 首重复音乐已跳过`)
  }
...
async function restoreUploadProgress() {
  try {
    ...
    if (pendingUploads.length > 0) {
      ...
      if (resumedUploads.length > 0) {
        ...
        if (uploadingTasks.length > 0) {
          ...
          toast.info(`检测到 ${uploadingTasks.length} 个后台正在上传的任务`)
        } else {
          toast.info(`检测到 ${resumedUploads.length} 个未完成的上传任务`)
        }
      }
    ...
    if (progress.length > 0) {
      ...
      toast.info(`检测到 ${progress.length} 个上传任务正在进行中`)
    }
  } catch (e) {
...
async function resumeUpload(fileItem) {
  ...
  try {
    ...
    toast.success(`${fileItem.name} 上传完成`)
    loadMusic()
    loadFilters()
  } catch (error) {
...
function handleReselectFile(fileItem) {
  ...
  input.onchange = async (e) => {
    ...
    if (file.name !== fileItem.name || file.size !== fileItem.size) {
      toast.warning(`请选择相同的文件: ${fileItem.name} (${formatFileSize(fileItem.size)})`)
      return
    }
    ...
    toast.success('文件已选择，点击"继续上传"按钮')
  }
...
async function cancelUpload() {
  ...
  if (uploadingFiles.length > 0) {
    toast.info(`已取消 ${uploadingFiles.length} 个文件的上传`)
  }
}
...
// 批量下载选中歌曲的歌词
function batchDownloadLyricsForSelected() {
  if (selectedSongs.value.length === 0) {
    toast.warning('请先选择音乐')
    return
  }
  showBatchLyricsConfirmDialog.value = true
}

async function confirmBatchDownloadLyrics() {
  showBatchLyricsConfirmDialog.value = false
  ...
  try {
    ...
    if (response.data.success) {
      ...
      toast.error('启动下载任务失败')
    }
  } catch (error) {
    console.error('批量下载歌词失败:', error)
    toast.error('批量下载歌词失败')
  } finally {
...
async function pollLyricsProgress() {
  ...
  try {
    ...
    if (task.status === 'pending' || task.status === 'running') {
      ...
      downloadingAllLyrics.value = false
      toast.success(`歌词下载完成：成功 ${task.success}，失败 ${task.failed}`)
...
:deep(.native-table .col-key-album),
