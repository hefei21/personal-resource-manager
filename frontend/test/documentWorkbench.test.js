...
test('document PC refinement keeps categories collapsed and preview controls usable', () => {
  ...
  assert.match(documents, /<NativeIcon name="list-dashes" size="15"/u)
  assert.match(detail, /\.document-detail-hero \{ padding: 16px 48px 16px 0;/u)
  assert.match(detail, /documentFileIcon\(document\.filePath\)/u)
  assert.match(detail, /documentFileTone\(document\.filePath\)/u)
  assert.match(detail, /\.document-action-card > span \{[^}]*color: var\(--color-primary\);[^}]*background: var\(--color-primary-surface\);/u)
  assert.doesNotMatch(detail, /\.document-action-card--primary > span/u)
...
test('document mobile refinement uses touch-native navigation and shared preview continuity', () => {
  ...
  assert.match(mobile, /row\.category\.fileCount/u)
  assert.ok(mobile.indexOf('class="mobile-search-actions"') < mobile.indexOf('class="mobile-browse-toolbar"'))
  ...
  assert.match(mobile, /class="mobile-upload-button"[\s\S]*aria-label="上传文档"/u)
  assert.match(mobile, /\.mobile-upload-button \{[\s\S]*width: 44px;[\s\S]*flex: 0 0 44px;/u)
  assert.match(mobile, /class="action-sheet-index-status"[\s\S]*ragStatusLabel\(currentDoc\?\.indexStatus\)/u)
  assert.doesNotMatch(mobile, /ragStatusLabel\(doc\.indexStatus\)/u)
  ...
  assert.match(mobile, /已显示全部 \{\{ total \}\} 项/u)
  assert.doesNotMatch(mobile, /mobile-list-pagination__track/u)
  ...
  assert.match(mobile, /--document-type-caption:\s*12px/u)
  assert.match(mobile, /--document-type-section:\s*16px/u)
  assert.match(mobile, /\.file-name \{[\s\S]*font-size: var\(--document-type-emphasis\)/u)
  assert.doesNotMatch(mobile, /:active[^\{]*\{[^\}]*background:\s*var\(--color-primary-surface\)/u)
