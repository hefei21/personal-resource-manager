...
test('PDF preview uses one pinned local PDF.js runtime on PC and mobile', () => {
  ...
  assert.match(runtimeSource, /import\('pdfjs-dist\/legacy\/build\/pdf\.mjs'\)/u)
  assert.match(runtimeSource, /legacy\/build\/pdf\.worker\.min\.mjs\?url/u)
  assert.match(runtimeSource, /openAuthenticatedPdfDocument/u)
  assert.match(runtimeSource, /export function openMobilePdfDocument/u)
  assert.match(runtimeSource, /useWorkerFetch: false/u)
  assert.match(runtimeSource, /isOffscreenCanvasSupported: false/u)
  assert.match(runtimeSource, /isImageDecoderSupported: false/u)
  ...
  assert.match(mobileSource, /openMobilePdfDocument/u)
