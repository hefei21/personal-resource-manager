...
test('deletes a category subtree transactionally and moves new and legacy documents to the surviving parent', options, () => { const db=setup(); try { assert.deepEqual(deleteDocumentCategoryTree(db,2),{deletedCategories:2,movedDocuments:3,categoryId:1}); assert.equal(db.prepare('SELECT COUNT(*) count FROM documents').get().count,3); assert.deepEqual(db.prepare('SELECT category_id,category,subcategory FROM documents WHERE id=9').get(),{category_id:1,category:'技术',subcategory:null}) } finally {db.close()} })
