...
const pcSource = fs.readFileSync(path.join(frontendRoot, 'pc', 'pages', 'BooksPC.vue'), 'utf8')
...
test('ebook upload confirmation cannot race metadata parsing on PC or mobile', () => {
  ...
  assert.match(pcSource, /:confirm-disabled="uploading \|\| parsingMetadata"/u)
  assert.match(pcSource, /if \(uploading\.value \|\| parsingMetadata\.value\) return/u)
