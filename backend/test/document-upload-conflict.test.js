...
test('route source exposes the frozen upload and version contracts', () => {
  ...
  assert.match(upload, /router\.post\('\/:id\/versions\/upload'.*documentUpload/u)
  assert.match(upload, /appendDocumentVersion\(\{/u)
  assert.match(upload, /scheduleDocumentIndex\(database, req\.params\.id, 'RAG_SOURCE_VERSION_CHANGED'\)/u)
  assert.match(upload, /assertDocumentVersionFileType\(db, target\.id, req\.file\.originalName\)/u)
  assert.match(upload, /assertDocumentVersionFileType\(database, req\.params\.id, req\.file\.originalName\)/u)
  assert.match(source, /DOCUMENT_VERSION_FILE_TYPE_MISMATCH/u)
