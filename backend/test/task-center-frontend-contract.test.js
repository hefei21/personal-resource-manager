...
test('Stage 3.4 frontend task center keeps the frozen API and shared-state boundaries', () => {
  ...
  const presentationSource = readFrontend('domain/taskPresentation.js')
  ...
  assert.match(viewSource, /ACTIVE_POLL_INTERVAL_MS\s*=\s*2000/u)
  assert.match(viewSource, /IDLE_POLL_INTERVAL_MS\s*=\s*15000/u)
  assert.match(viewSource, /visibilitychange/u)
  assert.match(viewSource, /clearTimeout\(pollTimer\)/u)
  assert.match(viewSource, /window\.setTimeout/u)
  ...
  assert.match(presentationSource, /pending[\s\S]*?leased[\s\S]*?running/u)
  assert.match(presentationSource, /SAFE_TASK_ERROR_CODE_PATTERN/u)
  assert.match(presentationSource, /任务失败（代码：\$\{safeCode\}）/u)
  assert.match(viewSource, /taskSourcePresentation/u)
  assert.match(viewSource, /taskStageLabel/u)
  assert.match(viewSource, /effectiveTaskStatus/u)
  assert.match(presentationSource, /RAG 文本索引/u)
  assert.match(presentationSource, /代码仓库克隆/u)
  assert.match(presentationSource, /Steam 游戏同步/u)
  assert.doesNotMatch(presentationSource, /其他任务|未关联资源/u)
  ...
  const taskTypeSource = [storeSource, viewSource, presentationSource].join('\n')
