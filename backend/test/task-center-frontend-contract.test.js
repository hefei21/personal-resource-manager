...
test('Stage 3.4 frontend task center keeps the frozen API and shared-state boundaries', () => {
  ...
  assert.doesNotMatch(viewSource, /POLL_INTERVAL_MS|visibilitychange|clearTimeout\(pollTimer\)|window\.setTimeout/u)
  assert.match(viewSource, /onBeforeUnmount/u)
  assert.match(viewSource, /ACTIVE_STATUS_SET/u)
  assert.match(viewSource, /@click="handleRefresh"/u)
  assert.match(viewSource, /void store\.fetch\(\)/u)
